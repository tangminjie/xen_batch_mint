...
describe("xenAttackV4", function () {
    async function deployOneYearLockFixture() {
        ...
        const MINIPROXY = await ethers.getContractFactory("MiniProxyV4");
        const miniProxy = await MINIPROXY.deploy();
        const miniProxyAddress = miniProxy.address;

        //const xenContractAddress = '0x06450dEe7FD2Fb8E39061434BAbCFC05599a6Fb8';
        const XenAttackV4 = await ethers.getContractFactory("xenAttackV4");
        const xenAttackV4 = await XenAttackV4.deploy(miniProxy.address);

        return { xenAttackV4, owner, otherAccount,miniProxyAddress };
    }
    ...
    //         const { xenAttackV4,miniProxyAddress } = await loadFixture(deployOneYearLockFixture);
    //         //console.log(xenContractAddress);
    //         expect(await xenAttackV4.miniProxyAddress()).to.equal(miniProxyAddress);
    //       });
    // });

    // describe("batch mintRank test",function(){
    //     it("batchMint with term < 0 ",async function(){
    //         const { xenAttackV4,owner,otherAccount,xenContractAddress } = await loadFixture(deployOneYearLockFixture);
    //         expect(await xenAttackV4.connect(otherAccount).batchMint(10,-1)).to.be.revertedWith("termDat error!");
    //     });
    // });

    describe("batch Claim test",function(){
        it("batchClaimWithXenContract call",async function(){
            const { xenAttackV4,owner,otherAccount,miniProxyAddress } = await loadFixture(deployOneYearLockFixture);
            let tx = await xenAttackV4.connect(otherAccount).batchMint(150,1);

           // console.log(`userInfo rerm:${userInfo.term}, userInfo rank: ${userInfo.rank}`);
            // increase and mine time so we can claimReward
            await increaseTime(24*60*60);
            //claim and transfer xen
            await xenAttackV4.connect(otherAccount).batchClaimWithXenContract(150);
            const xenContractAddress = '0x06450dEe7FD2Fb8E39061434BAbCFC05599a6Fb8';
